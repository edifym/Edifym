...
if __name__ == "__main__":
    ...
    sim_secs_static_nofp = CommandHelper.run_command_output(['awk', '/sim_sec/ {print $2}', f'stats_static_nofp.txt'], './').splitlines()
    ...
    sim_insts_static_nofp = CommandHelper.run_command_output(['awk', '/sim_insts/ {print $2}', f'stats_static_nofp.txt'], './').splitlines()
    ...
    if len(sim_secs_static_nofp) != 27:
        print(f'static_nofp stats incorrect stats length {len(sim_secs_static_nofp)} {sim_secs_static_nofp}')
        sys.exit(-1)
    ...
    total_sim_sec_static_nofp = 0
    total_sim_sec_dynamic = 0
    total_sim_insts_static = 0
    total_sim_insts_static_nofp = 0
    ...
    for i in range(26):
        if i % 2 == 0:
            ...
            print(f'adding {int(sim_secs_static[i][2:]):,} {int(sim_secs_static[i-1][2:]):,} {int(sim_secs_static[i][2:]) - int(sim_secs_static[i - 1][2:]):,} {int(sim_insts_static[i]) - int(sim_insts_static[i - 1]):,}')
    ...
    for i in range(26):
        if i % 2 == 0:
            prev_time = int(sim_secs_static_nofp[i][2:])
            prev_insts = int(sim_insts_static_nofp[i])
        else:
            print(f'adding {int(sim_secs_static_nofp[i][2:]):,} {int(sim_secs_static_nofp[i-1][2:]):,} {int(sim_secs_static_nofp[i][2:]) - int(sim_secs_static_nofp[i - 1][2:]):,} {int(sim_insts_static_nofp[i]) - int(sim_insts_static_nofp[i - 1]):,}')
            total_sim_sec_static_nofp += int(sim_secs_static_nofp[i][2:]) - int(sim_secs_static_nofp[i - 1][2:])
            total_sim_insts_static_nofp += int(sim_insts_static_nofp[i]) - int(sim_insts_static_nofp[i - 1])
    ...
    for i in range(26):
        if i % 2 == 0:
            ...
            print(f'adding {int(sim_secs_dynamic[i][2:]):,} {int(sim_secs_dynamic[i-1][2:]):,} {int(sim_secs_dynamic[i][2:]) - int(sim_secs_dynamic[i - 1][2:]):,} {int(sim_insts_dynamic[i]) - int(sim_insts_dynamic[i - 1]):,}')
    ...
    print(f'static_nofp: {total_sim_sec_static_nofp} - {total_sim_insts_static_nofp}')
